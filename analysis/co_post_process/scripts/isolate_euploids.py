...
def obtain_mat_meio(
    ...
    amplification_failures = (
        aneuploidy_df.filter(~pl.col("uid").is_in(day3_null_embryos))
        .group_by("uid")
    ...
    aneuploidy_df = aneuploidy_df.filter(
        ~pl.col("uid").is_in(day3_null_embryos)
    ).filter(~pl.col("uid").is_in(amplification_failures))
    maternal_meiotic_uids = (
        aneuploidy_df.filter(
            (pl.col("post_max") > ppThresh) & (pl.col("bf_max_cat").is_in(["3m", "1p"]))
        )
        .filter(
            ~(
