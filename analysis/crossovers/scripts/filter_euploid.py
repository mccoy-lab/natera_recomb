...
if __name__ == "__main__":
    ...
    true_euploid_data = (
        trios_data_euploid[trios_data_euploid.bf_max_cat]
        .groupby(["mother", "father", "child"])
        .count()
        .reset_index()
    )
    df = true_euploid_data.pivot_table(
        index=["mother", "father"], aggfunc="size"
    ).reset_index()
    parent_dict_count = {
        f"{m}+{f}": z
        for (m, f, z) in zip(df.mother.values, df.father.values, df[0].values)
    }
    x = np.zeros(true_euploid_data.shape[0])
    for i, (m, f) in enumerate(
        zip(true_euploid_data.mother.values, true_euploid_data.father.values)
    ):
        if f"{m}+{f}" in parent_dict_count:
            x[i] = parent_dict_count[f"{m}+{f}"]
    true_euploid_data["n_embryos"] = x
    euploid_trios_df = true_euploid_data[true_euploid_data.n_embryos >= 3][
        ["mother", "father", "child"]
    ]
